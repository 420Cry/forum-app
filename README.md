...
- **Supabase cookies** — `cookieOptions.secure` is `false` in development so session cookies work over `http://app.forum.test`. Production sets `secure` + `httpOnly` so tokens are not readable via `document.cookie`.
