...
import { STARTUP_STAGE_LABELS, type StartupStage } from '~/types/post'
...
type PillVariant = NonNullable<VariantProps<typeof pill>['variant']>

const props = withDefaults(
  defineProps<{
    // Funding stage or category. Omit for category tags → base styling.
    // `base` is the omit-default, so it's excluded from the public prop.
    variant?: Exclude<PillVariant, 'base'>
  }>(),
  {
    variant: undefined,
  },
...
const resolvedVariant = computed<PillVariant>(() => props.variant ?? 'base')

// Fall back to a default label when no slot content is provided. Category
// tags (base, no variant) and opportunity are expected to pass their own text.
